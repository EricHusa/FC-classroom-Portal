<template>
  <div>
    ...
    <b-row class="my-1" v-for="item in fields" :key="item.key">
      <b-col sm="3">
        <label :for="`type-${item.name}`"><code>{{ item.name }}</code>:</label>
      </b-col>
      <b-col sm="9">
        <b-form-input
          :id="`type-${item.type}`"
    ...
    block>
      <b-dropdown-form>
         <b-form-checkbox-group
          ...
          <b-collapse id="experiment-update-section" :visible="!checkRole() && experiment.id">
              <b-row>
                  <b-col sm="3">
              <b-button
          @click="deleteExperiment()"
          variant="danger"
              >Delete</b-button></b-col>
              <b-col sm="9"><b-button
          @click="updateExperiment()"
          variant="warning"
          >Update info</b-button></b-col>
          </b-row>
          ...
          <b-collapse id="experiment-create-section" :visible="!checkRole() && !experiment.id">
              <b-button
